<?php
    ...
    function editStudent($id,$xml,$conn){
        ...
        if($_POST["newIX_$id"] !== null && preg_match_all("[12]{1}[0-9]{3}\\/[0-9]{6}",$_POST["newIX_$id"])){
            $query = sprintf("UPDATE student SET index = '%s' WHERE student_id = '%s';",mysqli_real_escape_string($conn,$_POST["newIX_$id"]),mysqli_real_escape_string($conn,$id));
            $conn->query($query) or showErrorAlert($xml->errors->wrong_index[0]);
        }

        if($_POST["newUE_$id"] !== null){
            $query = sprintf("UPDATE student SET email = '%s' WHERE student_id = '%s';",mysqli_real_escape_string($conn,$_POST["newUE_$id"]),mysqli_real_escape_string($conn,$id));
