...
public class ServerController {
    ...
    @PostMapping("insert/student") //Implemented iOS
    @CrossOrigin(origins = {"https://localhost:62812"})
    ...
    @RequestMapping(value = "checkPassword/student/{index}", method = RequestMethod.POST) //Implemented iOS
    @CrossOrigin(origins = {"https://localhost:62812"})
    ...
    @RequestMapping(value = "getStudentName/{index}", method = RequestMethod.GET) //Implemented iOS
    @CrossOrigin(origins = {"https://localhost:62812"})
    String getNameSurnameStudent(@PathVariable String index){
        return this.serverService.getNameSurnameStudent(index);
    }

    @RequestMapping(value = "getCourseData/{index}", method = RequestMethod.GET) //Implemented iOS
    @CrossOrigin(origins = {"https://localhost:62812"})
    List<CourseDataInstance> getCourseData(@PathVariable String index){
        return this.serverService.getCourseData(index);
    }

    @RequestMapping(value = "recordAttendance/{index}/{subjectId}/{isExercise}", method = RequestMethod.GET) //Implemented iOS
    @CrossOrigin(origins = {"https://localhost:62812"})
